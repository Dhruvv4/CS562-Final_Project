        ...
        for i in range(MF_Struct['groupingVariables']):
            ...
            if i == 0:
                if (prod, month) in sales_gb_group:
                    sales_gb_group[(prod, month)]['1_avg'] += quantC
                    sales_gb_group[(prod, month)]['1_max'] = max(sales_gb_group[(prod, month)]['1_max'], quantC)
                else:
                    sales_gb_group[(prod, month)] = {}
                    sales_gb_group[(prod, month)]['1_avg'] = quantC
                    sales_gb_group[(prod, month)]['1_max'] = quantC
    
    for key, values in sales_gb_group.items():
        if len(key) == 2:
            if sales_gb_group[(key[0],key[1])]['1_avg'] > sales_gb_group[(key[0],key[1])]['1_max']:
        
                output.add_row([*key] + [sales_gb_group[(key[0],key[1])]['1_avg'],sales_gb_group[(key[0],key[1])]['1_max'],])
